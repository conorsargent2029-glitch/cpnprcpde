...
------------------------------------------------------------------------
GOT A FOLDER WITH ONLY .bin FILES, NO .txt EXPORT?
------------------------------------------------------------------------
Use the separate chi_bin_to_excel.py script instead (same folder as this
one) -- it reads CHI's .bin saves directly, for when CHI's auto-text-save
setting was off (e.g. an unattended overnight run) and there's no .txt to
parse. See that script's own docstring for details.
"""
...
def estimate_peak_from_curve(curve, potentials, edge_points=8):
    ...
    curve, for when CHI's own peak-picked value isn't available and has to
    be re-derived from the raw curve instead. Fits a straight line through
    the first/last edge_points (the flat baseline on either side of the
    peak) and returns the curve value with the largest deviation from that
    line -- the standard by-hand way of reading peak height off a
    voltammogram.
...
if __name__ == "__main__":
    if len(sys.argv) >= 3 and sys.argv[1] != "--auto":
        ...
        print("  (for a folder of .bin-only saves with no .txt export, use chi_bin_to_excel.py instead)")
